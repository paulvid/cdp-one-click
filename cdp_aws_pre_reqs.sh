...
then
    # Handling miss configurations
    if [[ "$cloud_provider" == "az" ]]
    then
        handle_exception 1 $prefix "credential generation" "Azure is not supported yet"
    fi
   
    if [[ "$external_id" == "not_provided" ]]
    then
        handle_exception 1 $prefix "credential generation" "External ID was not provided"
    fi
