...
then
    ...
    id=$(curl -s https://cloudbreak-imagecatalog.s3.amazonaws.com/v3-prod-cb-image-catalog.json | jq -r '.images | ."cdh-images" | sort_by(.created) | reverse[] | select (.version=="'${dl_version}'" and .images.gcp?) | .uuid' | head -1)
...
cdp datahub create-gcp-cluster --cluster-name ${cluster_name} \
