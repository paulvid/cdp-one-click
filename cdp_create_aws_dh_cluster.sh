...
then
    ...
    id=$(curl -s https://cloudbreak-imagecatalog.s3.amazonaws.com/v3-prod-cb-image-catalog.json | jq -r '.images | ."cdh-images" | sort_by(.created) | reverse[] | select (.version=="'${dl_version}'" and .images.aws?) | .uuid' | head -1)
...
for row in $(echo ${def_template} | jq -r '.instanceGroups[] | @base64'); do
    ...
    enable_encryption=$(_jq  '.volumeEncryption.enableEncryption')
    recipes=''
    ...
    instance_groups=$instance_groups" nodeCount=${node_count},instanceGroupName=${instance_group_name},instanceGroupType=${instance_group_type},instanceType=${instance_type},rootVolumeSize=${root_volume_size},attachedVolumeConfiguration=[{volumeSize=${volume_size},volumeCount=${volume_count},volumeType=${volume_type}}],${recipes}recoveryMode=${recovery_mode},volumeEncryption={enableEncryption=false} " 
done
